...
int main (int argc, char **argv)
{
    ...
    if ((fd = open((argc>1)?argv[1]:DEFAULT_DEVICE, O_RDONLY)) < 0) {
        perror("open device");
        return 1;
    }
    ...
    if ((sockfd = getsockfd(host, port)) < 0) {
        fprintf(stderr, "Error: getsockfd(%s,%s)\n", host, port);
        return 1;
    }
    sock = fdopen(sockfd, "a");
    ...
    while (1) {
        ...
        if (button[1] && !b[1]) {
            b[1] = 1;
            fprintf(sock, "asserv stop\n");
        } else if (button[3] && !b[2]) {
            b[2] = 1;
            fprintf(sock, "asserv rot -9000\n");
