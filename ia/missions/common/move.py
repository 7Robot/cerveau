...
class MoveMission(Mission):
    ...
    def _set_pos(self, pos):
        self.logger.debug(u"[real] pos: %d %d" %(pos.x, pos.y))
        self._pos = pos
    def _get_pos(self):
        return self._pos
    ...
    def _set_target_pos(self, target_pos):
        self.logger.info(u"[target] pos: %d %d" %(target_pos.x, target_pos.y))
    ...
    def _set_rot(self, rot):
        self.logger.debug(u"[real] rot: %d" %rot)
        self._rot = rot
    def _get_rot(self):
        return self._rot
    ...
    def _set_target_rot(self, target_rot):
        self.logger.info(u"[target] rot: %d" %target_rot)
    ...
    def _set_mission(self, mission):
        self.logger.info(u"[mission] %s -> %s"
    ...
    def reach_y(self, callback, y):
        if self.mission == None:
            ...
            print u"rot: %d, target_rot: %d, dtheta: %d" %(self.rot,
                self.target_rot, dtheta)
            print u"pos: ", self.pos
            print u"target: ", self.target_pos
            self.target_pos += Vertex(dist * cos(self.rot/18000*pi), dist * sin(self.rot/18000*pi))
            print u"target: ", self.target_pos
    ...
    def stop(self, callback):
        if self.mission == u"speed":
            ...
            self.missions[u"speed"].stop(callback)

    ### FINDESMISSIONS ###

    def process_event(self, event):
        if self.mission == u"forward" or self.mission == u"rotate" \
                or ((self.mission == u"speed" or self.mission == u"speed_target") and self.state == u"stopping"):
            if event.name == self.mission and event.type == u"done":
                if self.mission == u"speed":
