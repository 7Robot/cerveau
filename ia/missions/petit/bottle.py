# -*-coding:UTF-8 -*

from missions.mission import Mission
from robots.robot import Robot

class BottleMission(Mission):
    def __init__(self, robot, can, ui):
        super(self.__class__,self).__init__(robot, can, ui)

    def start(self):
        if self.state == 0:
            self.create_timer(500)
            self.state += 1
            

    def process_event(self, e):
        if self.state == 1:
            if e.name == u'timer':
                self.state += 1
                self.missions[u"forward"].start(self, 2700)
                
        if self.state == 2:
            if e.name == u"forward" and e.type == u"done":
                self.state += 1
                if Robot.side == u"violet":
                    self.missions[u"rotate"].start(self, -9300) # -9200
                else:
                    self.missions[u"rotate"].start(self, -8900)
                                    
        elif self.state == 3:
            if e.name == u"rotate" and e.type == u"done":
                self.state += 0.5
                self.create_timer(3000)
                #self.missions["speed"].start(80)
                self.missions[u"forward"].start(self, 13000) # TODO check avant totem 2
                
        elif self.state == 3.5:
            if e.name == u"forward" and e.type == u"done":
                self.state += 0.5
                for i in [1,2]: self.missions[u"threshold"].activate(i, False)
                self.missions[u"speed"].start(50)
                self.create_timer(5000) # si on bump pas
                    
        elif self.state == 4:
            if (e.name == u"bump" and e.state==u"close") or e.name == u"timer":
                self.state += 0.5
                self.create_timer(1500, timername=u"recalage")
                
                
        elif self.state == 4.5:
            if e.name == u"timer":
                self.state += 0.5
                self.missions[u"speed"].stop(self)
        
        elif self.state == 5:
            if e.name == u"speed" and e.type == u"done":
                self.state += 1        
                for i in [1,2]: self.missions[u"threshold"].activate(i, True)
                self.missions[u"forward"].start(self, -1300)
                
        elif self.state == 6:
            if e.name == u"forward" and e.type == u"done":
                self.state +=1
                if Robot.side == u"violet":
                    self.missions[u"rotate"].start(self, 9000)
                else:
                    self.missions[u"rotate"].start(self, 9000)

        elif self.state == 7:
            if e.name == u"rotate" and e.type == u"done":
                self.state += 0.5
                # Avance de 142 cm
#                self.missions["double_chemin"].start(self, 14400, -4900, -9000) # 14200, 14400
                # J'ai mesuré 124cm avec un metre, tu aurais pas inversé le 2 et
                # le 4 ?
                if Robot.side == u"violet":
                    self.missions[u"double_chemin"].start(self, 13500, -4900, -9000)
#                    self.missions["forward"].start(self, 13000) # 14400
                else:
                    self.missions[u"double_chemin"].start(self, 13500, -4900, -9000)
#                    self.missions["forward"].start(self, 13650) # 14400

        elif self.state == 7.5:
            if e.name == u"double_chemin" and e.type == u"done":
#            if e.name == "forward" and e.type == "done":
                self.state += 0.5
                self.missions[u"forward"].start(self, -1000) # -2000
                
        elif self.state == 8:
            if e.name == u"forward" and e.type == u"done":
                self.state += 1
                self.missions[u"rotate"].start(self, -9000)
                    
        elif self.state == 9:
            if e.name == u"rotate" and e.type == u"done":
                self.state += 1
                for i in [1,2]: self.missions[u"threshold"].activate(i, False)
                self.missions[u"speed"].start(50)
                self.create_timer(6000) # si on bump pas

                
        elif self.state == 10:
            if (e.name == u"bump" and e.state==u"close") or e.name == u"timer":
                self.state += 0.5
                self.create_timer(1500, timername=u"recalage")
        
        elif self.state == 10.5:
            if e.name == u"timer":        
                self.state += 0.5
                self.missions[u"speed"].stop(self)
                
        elif self.state == 11:
            if e.name == u"speed" and e.type == u"done":
                self.state += 1
                for i in [1,2]: self.missions[u"threshold"].activate(i, True)
            
                # Missions secondaires
                if not(self.missions[u"double_chemin"].went_B):
                   print u"Lingot milieu"
                   self.missions[u"lingot_milieu"].start()
                else:
                   print u"rase totem "
                   self.missions[u"rase_totem"].start()
            
